...
import { GanttModifiedRowDto } from 'projects/mon-impianti/src/app/@services/dtos/gantt.dto';
import { GanttProjectItem, ProjectDates } from 'projects/mon-impianti/src/app/@services/dtos/progetto/progetto.detail.dto';
...
import { GanttItemAtemporal, ItemsMap as AtemporalMap } from './gantt-item-atemporal.model';
import { GanttItemExecution, ItemsMap as ExecutionMap, ItemUpdater } from './gantt-item-execution.model';
import { GanttItemMultiProject } from './gantt-item-multiproject.model';
import { DateType } from './gantt-item-project.model';
import { GanttItemTemporal, ItemsMap as TemporalMap } from './gantt-item-temporal.model';
...
export const isMoment = (value: DateType): value is moment.Moment => !!value && (value as moment.Moment).daysInMonth !== undefined;
export const stringifyDate = (date: DateType): string => isMoment(date) && date?.isValid() ? date.format('YYYY-MM-DD') : null;
export const stringToDate = (date: string): Date => date ? moment(date, 'YYYY-MM-DD').toDate() : null;
export const stringToMoment = (date: string): moment.Moment =>
  date && moment(date, 'YYYY-MM-DD').isValid() ? moment(date, 'YYYY-MM-DD') : null;

export const atemporalRowToModifiedRow = (source: GanttItemAtemporal): GanttModifiedRowDto => ({
  ...source,
...
export const temporalRowToModifiedRow = (source: GanttItemTemporal): GanttModifiedRowDto => ( {
    ...source,
    mapStartDate: stringifyDate(source.mapStartDate),
    mapEndDate: stringifyDate(source.mapEndDate),
...
export const executionRowToModifiedRow = (source: GanttItemExecution): GanttModifiedRowDto => ( {
    ...source,
    baselineStartDate: stringifyDate(source.baselineStartDate),
    baselineEndDate: stringifyDate(source.baselineEndDate),
...
export const applyUpdaterExecution = (source: GanttItemExecution, updater: ItemUpdater): GanttItemExecution => {
    const item = { ...source };

    item.actualStartDate = updater.actualStartDate != null ? moment(updater.actualStartDate) : null;
    item.actualEndDate = updater.actualEndDate != null ? moment(updater.actualEndDate) : null;
    if (item.level <= 2) {
        item.baselineStartDate = updater.baselineStartDate != null ? moment(updater.baselineStartDate) : null;
        item.baselineEndDate = updater.baselineEndDate != null ? moment(updater.baselineEndDate) : null;
        item.forecastStartDate = updater.forecastStartDate != null ? moment(updater.forecastStartDate) : null;
        item.forecastEndDate = updater.forecastEndDate != null ? moment(updater.forecastEndDate) : null;
        item.startMonth = updater.startMonth;
        item.duration = updater.duration;
        item.mapStartDate = updater.mapStartDate && moment(updater.mapStartDate) || item.mapStartDate;
        item.mapEndDate = updater.mapEndDate && moment(updater.mapEndDate) || item.mapEndDate;
        item.lastReauthStartDate = updater.lastReauthStartDate && moment(updater.lastReauthStartDate) || item.lastReauthStartDate;
        item.lastReauthEndDate = updater.lastReauthEndDate && moment(updater.lastReauthEndDate) || item.lastReauthEndDate;
    }
...
export const applyUpdaterTemporal = (source: GanttItemTemporal, updater: ItemUpdater): GanttItemTemporal => {
    const item = { ...source };
    item.baselineStartDate = updater.baselineStartDate && moment(updater.baselineStartDate) || item.baselineStartDate;
    item.baselineEndDate = updater.baselineEndDate && moment(updater.baselineEndDate) || item.baselineEndDate;
    item.weight = updater.weight != null && updater.weight || item.weight;
    item.progress = updater.progress != null && updater.progress || item.progress;
    item.startMonth = updater.startMonth;
    item.duration = updater.duration;
...
export const modifiedRowsToAtemporalGantt = (
    ...
    gantt: AtemporalInput): AtemporalInput => {
    ...
    modifiedRows.forEach(row => {
        ...
        if (mapItem) {
            ...
            item.mapStartDate = row.mapStartDate && moment(row.mapStartDate) || item.mapStartDate;
            item.mapEndDate = row.mapEndDate && moment(row.mapEndDate) || item.mapEndDate;
...
export const modifiedRowsToExecutionGantt = (modifiedRows: GanttRow[], gantt: ExecutionInput): ExecutionInput => {
...
export const modifiedRowsToTemporalGantt = (modifiedRows: GanttRow[], gantt: TemporalInput): TemporalInput => {
...
export const modifiedRowsToMultiProjectGantt = (modifiedRows: GanttRow[], gantt: ExecutionInput): ExecutionInput => {
    const itemsMap: ExecutionMap = toMapExecution([ ...(gantt?.ganttRows || []) ]);
    modifiedRows.forEach(row => {
        const mapItem = itemsMap.get(row.id);
        if (mapItem) {
...
export const applyUpdaterMultiProject = (source: GanttItemMultiProject, updater: ItemUpdater): GanttItemMultiProject => {
    const item = { ...source };

    item.actualStartDate = !!updater.actualStartDate && moment(updater.actualStartDate) || null;
    item.actualEndDate = !!updater.actualEndDate && moment(updater.actualEndDate) || null;
    if (item.level <= 2) {
        item.baselineStartDate = !!updater.baselineStartDate && moment(updater.baselineStartDate) || null;
        item.baselineEndDate = !!updater.baselineEndDate && moment(updater.baselineEndDate) || null;
        item.forecastStartDate = !!updater.forecastStartDate && moment(updater.forecastStartDate) || null;
        item.forecastEndDate = !!updater.forecastEndDate && moment(updater.forecastEndDate) || null;
        item.mapStartDate = !!updater.mapStartDate && moment(updater.mapStartDate) || item.mapStartDate;
        item.mapEndDate = !!updater.mapEndDate && moment(updater.mapEndDate) || item.mapEndDate;
    }
    item.weight = updater.weight != null && updater.weight || item.weight;
...
export const projectMapItemMapper = (input: GanttProjectItem<moment.Moment>): GanttItemAtemporal => ({
        id: input.id,
        name: input.name,
...
export const mapAtemporalRows = (input: GanttRow[]): ProjectDates<moment.Moment>[] => {
    const fnMapper = (_input: GanttRow): ProjectDates<moment.Moment> => ({
        id: _input.id,
        plannedStartDate: null,
        plannedEndDate: null,
        mapStartDate: !!_input.mapStartDate?.isValid() ? moment(_input.mapStartDate) : null,
