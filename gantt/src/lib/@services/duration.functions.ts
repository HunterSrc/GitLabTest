...
export const computeEnd = (duration: Interval): number => duration.start + duration.duration;

// Date due durate ritorna quella che inizia per prima
export const getMin = (d1: Interval, d2: Interval): Interval => d1.start <= d2.start ? d1 : d2;

// Date due durate ritorna quella che inizia per ultima
export const getMax = (d1: Interval, d2: Interval): Interval => getMin(d1, d2) === d1 ? d2 : d1;
...
export const getOverlapOrdered = (first: Interval, second: Interval): Interval => {
    const firstEnds = computeEnd(first);
    return { start: firstEnds > second.start ? second.start : firstEnds , duration: firstEnds - second.start };
};

export const getOverlap = (p1: Interval, p2: Interval): Interval => getOverlapOrdered(getMin(p1, p2), getMax(p1, p2));
...
export const sortPhaseDurations = (toSort: Interval[]): Interval[] => toSort.sort((p1, p2) => p1.start - p2.start);
...
export const mergePhaseDurationSequence = (toMerge: Interval[]): Interval =>
        sortPhaseDurations(toMerge)
            ...
            .reduce((p1, p2, i) => mergePhaseDurations(p1, p2), { start: 1, duration: 0 });
...
export const mergeDurationAndPoints = (d: Interval, points: number[] = []): Interval =>  {
