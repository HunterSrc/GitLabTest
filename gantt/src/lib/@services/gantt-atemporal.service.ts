...
import { applyEndToStartConstraint, buildConstraint, resolveEndToStartConstraint } from './constraints.functions';
...
export class GanttAtemporalService {
    ...
    private itemsSubject: ReplaySubject<GanttItemAtemporal[]> = new ReplaySubject();
    ...
    init(ganttRows: GanttItemAtemporal[], constraints: ConstraintDescriptor[]): void {
        this.itemsMap = ganttFn.toMap([...ganttRows]);
        this.projectConstraintsDescriptors = constraints;
        this.projectId = ganttRows.find(item => item.typeCode === 'NR_PROJECT')?.id;
    ...
    calculateProjectDuration(): void {
        ...
        const prjDuration = ganttFn.calculateProjectDuration(items, ganttFn.getItemsToExclude(items));
        ...
        project.mapEndDate = ganttFn.calculateProjectEndDate(items, ganttFn.getItemsToExclude(items));
        project.isActive = !!project.duration;
        this.itemsMap.set(project.id, project);
    }

    updateMapItems(sourceMap: ItemsMap, isAtemporal: boolean): void {
        this.recalculateMap(sourceMap, isAtemporal)
            .forEach((value, key) => { this.itemsMap.set(key, value); });
        this.itemsSubject.next(this.getItemsAsArray());
    }
    ...
    recalculateMap(sourceMap: ItemsMap, isAtemporal: boolean): ItemsMap {
        ...
        const result = new Map(ganttFn.computeGantt(items, isAtemporal).map(item => [item.id, item]));
        ...
        for (const constraintDescriptor of this.projectConstraintsDescriptors) {
            const constraint = buildConstraint(items.map(item => item.id), result, constraintDescriptor);
            if (constraint) {
                const slavePhase = { ...result.get(constraint.slavePhase?.id)};
                const constraintResult = resolveEndToStartConstraint(constraint);
                if (!!slavePhase) {
                    applyEndToStartConstraint(slavePhase, constraintResult, ganttStartDate, isAtemporal);
    ...
    getHeader(minCols: number): string[] {
        ...
        const cols = new Array(duration > minCols ? duration : minCols).fill(1).map((v, i) => v + i);
        if (this.isATemporal) {
            return cols;
        } else {
    ...
    updateLockedTypeCodeList(): void  {
        ...
        for (const constraintDescriptor of this.projectConstraintsDescriptors) {
            if ([ ...(constraintDescriptor.masterPhases || [])].some(item => activeItems.has(item))) {
