import { GanttBar, GanttItemExecution, GanttTaskBars, ItemsMap } from '../@model/gantt-item-execution.model';
...
export function getGridHeader(commission: GanttItemExecution): string[] {
    const start: MtDate = getMinItemDate(commission).isValid() ? getMinItemDate(commission) : moment();
    const end: MtDate = getMaxItemDate(commission);
    const monthsDuration = (end.diff(start, 'months') + 2) || 1 ;
    return new Array(monthsDuration).fill(1).map((val, index) => start.add(!!index && val, 'months').format('MM/YY'));
}

export function resolveCommissionDates(commissionId: number, map: ItemsMap): void {
...
export const calculateTodayCol = (  gridStart: moment.Moment,
                                    colSpan: number = 4,
                                    paddingFn: (day: number) => number = getQuarterEnd): number => {
...
export const calculateBar = (
    ...
    colSpan: number = 4): GanttBar => {
        ...
        const start = snapToToday ?  calculateTodayCol(gridStart) : (monthsDiff * colSpan) + paddingStart + 1;
...
export const calculateBars = (
    ...
    inactiveColor: string): GanttTaskBars => {
        const resolveColor = (color: string) => item.isActive && color || inactiveColor;
        const sameAsToday = (date: moment.Moment) => moment()?.format('DD/MM/YYYY') === date?.format('DD/MM/YYYY');
        const hasBaseline = (item.baselineStartDate as moment.Moment)?.isValid() &&
            (item.baselineEndDate as moment.Moment)?.isValid();
        const hasForecast = (item.forecastStartDate as moment.Moment)?.isValid() &&
            (item.forecastEndDate as moment.Moment)?.isValid();

        const hasMap = (item.mapStartDate as moment.Moment)?.isValid() &&
            (item.mapEndDate as moment.Moment)?.isValid();
        const hasLastReauth = (item.lastReauthStartDate as moment.Moment)?.isValid() &&
            (item.lastReauthEndDate as moment.Moment)?.isValid();
        let mapBar;
        let lastReauthBar;

        const hasActualStart = (item.actualStartDate as moment.Moment)?.isValid();
        const actualEnd = (item.actualEndDate as moment.Moment)?.isValid() && item.actualEndDate || moment();
        ...
        const actualBar = hasActualStart &&
            calculateBar(
                item.actualStartDate as moment.Moment,
                actualEnd as moment.Moment,
                gridStartDate,
                !!item.actualStartDate && !!item.actualEndDate && item.progress === 100 ? actualCompletedColor : resolveColor(actualColor),
...
export const makeStripedBar = (color: string): string => `repeating-linear-gradient(\
    -45deg,\
    white,\
    white 0.5rem,\
    ${color} 0.1rem,\
