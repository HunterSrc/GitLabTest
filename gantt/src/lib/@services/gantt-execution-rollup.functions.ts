 ...
 */
export const updateProgress = (item: GanttItemExecution, children: GanttItemExecution[]): void => {
    const getAddendum = (i: GanttItemExecution) => (i.weight / 100) * i.progress;
    item.progress = Math.round(children.reduce((a, c) => a + getAddendum(c), 0));
};
 ...
 * @param children i figli dell'elemento target
 */
export const updateActualDates = (item: GanttItemExecution, children: GanttItemExecution[]): void => {
        const startDates = children.map(child => child.actualStartDate as moment.Moment).filter(d => d?.isValid());
        const endDates = children.map(child => child.actualEndDate as moment.Moment).filter(d => d?.isValid());
        item.actualStartDate = startDates.length && moment(getMinDate(...startDates)) || null;
        item.actualEndDate = (endDates.length === children.filter(c => !!c.weight).length) && moment(getMaxDate(...endDates)) || null;
};
 ...
 */
export const updateForecast = (item: GanttItemExecution, children: GanttItemExecution[]): void => {
        ...
        if (item.actualStartDate && !item.actualEndDate) {
            const baselineLength = (item.baselineEndDate as moment.Moment)?.diff(item.baselineStartDate, 'days') || 0;
            const progressRemains = (100 - item.progress) / 100;
            item.forecastStartDate = moment(item.actualStartDate);
            item.forecastEndDate = item.level >= 2 ?
                moment().add(Math.round(progressRemains * baselineLength), 'days') :
                getMaxEndDateForecast(children);
        }
        if (!item.actualStartDate && !item.actualEndDate) {
            if (item.typeCode === 'ENTRY_INTO_SERVICE') {
                item.forecastStartDate = item.forecastStartDate ? moment(item.forecastStartDate) : moment(item.baselineStartDate);
                item.forecastEndDate = item.forecastEndDate ? moment(item.forecastEndDate) : moment(item.baselineEndDate);
            } else {
               if (item.level < 2) {
                    item.forecastStartDate = getMinStartDateForecast(children);
                    item.forecastEndDate = getMaxEndDateForecast(children);
               } else {
                if (moment(item.baselineStartDate).isBefore(moment())) {
                    const baselineLength = (item.baselineEndDate as moment.Moment)?.diff(item.baselineStartDate, 'days') || 0;
                    item.forecastStartDate = moment();
                    const shiftedEndDate = moment().add(baselineLength, 'days');
                    item.forecastEndDate = item.level >= 2 ? shiftedEndDate : getMaxEndDateForecast(children);
                } else {
...
export const calculateItemRollup = (item: GanttItemExecution, children: GanttItemExecution[]) => {
...
const getChildren = (itemId: number, itemsMap: ItemsMap, includeWeightZero: boolean = true): GanttItemExecution[] => {
    const includeItem = (item: GanttItemExecution) => item.isActive && (includeWeightZero || !!item?.weight);
    if (itemsMap.has(itemId)) {
        return itemsMap.get(itemId).childrenIdList?.map(id => getItem(id, itemsMap)).filter(includeItem);
    }
    return [];
};
...
export const calculateRollup = (itemId: number, itemsMap: ItemsMap) => {
    ...
    const children = getChildren(!!parent && parent.id || item.id, itemsMap, false);
...
export const extractIds = (items: GanttItemExecution[]): number[] => {
    ...
    return items.filter(item => !item.childrenIdList?.length)
...
export const updateMap = (itemsMap: ItemsMap, constraintDescriptors: ConstraintDescriptor[]): void => {
    extractIds(Array.from(itemsMap.values())).forEach((node: number) => calculateRollup(node, itemsMap));
    applyConstraints(itemsMap, constraintDescriptors);
    if(!isCommessaNominativa(itemsMap)){
        calculatePhaseForecast(itemsMap);
    }
    alignCommissionDuration(itemsMap);    
};

const alignCommissionDuration = (itemsMap: ItemsMap): void => {
    const commission = { ...[...(itemsMap.values() || [])].find(item => item.level === 1)};
    const phases = [...(itemsMap.values() || [])].filter(item => item.level === 2);
...
const isCommessaNominativa = (itemsMap: ItemsMap) =>{
    const commission = { ...[...(itemsMap.values() || [])].find(item => item.level === 1)};
    let isNominativa = commission.code.startsWith("N");
    return isNominativa;
}

const calculatePhaseForecast = (itemsMap: ItemsMap) => {
    const phases = [...(itemsMap.values() || [])].filter(item => item.level === 2 && item.childrenIdList.length === 0 && item.typeCode !== TypeCodeEnum.ENTRY_INTO_SERVICE);
    phases.forEach(phase => {
        let phaseUpdated = updateForecastDates(phase)
        itemsMap.set(phaseUpdated.id, phaseUpdated);
    });
}

const updateForecastDates = (item: GanttItemExecution) => {
    //aggiunta per le date forecast da aggiornare anche per
    //le fasi che non hanno figli e quindi non hanno logiche di rollup
    if (item.actualStartDate && item.actualEndDate) {
        //se ci sono inizio e fine attuali, le forecast le rispecchiano
        let item2 = {...item};
        item2.forecastStartDate = moment(item.actualStartDate);
        item2.forecastEndDate = moment(item.actualEndDate);
        item = item2;
    }
    if (item.actualStartDate && !item.actualEndDate) {
        //se c'è inizio ma non fine la fine viene calcolata
        let item2 = {...item};
        const baselineLength = (item.baselineEndDate as moment.Moment)?.diff(item.baselineStartDate, 'days') || 0;
        const progressRemains = (100 - item.progress) / 100;
        item2.forecastStartDate = moment(item.actualStartDate);
        item2.forecastEndDate = moment().add(Math.round(progressRemains * baselineLength), 'days');
        item = item2;
    }
    if (!item.actualStartDate && !item.actualEndDate) {
        //se etrambe non sono popolate dipende da se l'inizio
        //è prima o dopo la data odierna
        if (moment(item.baselineStartDate).isBefore(moment())) {
            //è prima quindi lo start è oggi e l'end è calcolato
            const baselineLength = (item.baselineEndDate as moment.Moment)?.diff(item.baselineStartDate, 'days') || 0;
            let item2 = {...item};
            item2.forecastStartDate = moment();
            item2.forecastEndDate = moment().add(baselineLength, 'days');
            item = item2;
        } else {
            //è dopo quindi sono uguali alle pianificate
            let item2 = {...item};
            item2.forecastStartDate = moment(item.baselineStartDate);
            item2.forecastEndDate   = moment(item.baselineEndDate);
            item = item2;
        }           
    }
    return item;
...
const getPhasesByTypecode = (itemsMap: ItemsMap, phaseIdsMap: Map<string, number>, phases: Set<string>): GanttItemExecution[] =>
    [...(phases.values() || [])]
...
const isSlavePhaseValid = phase => !!phase?.isActive && !phase.actualStartDate;

export const applyConstraints = (itemsMap: ItemsMap, constraintDescriptors: ConstraintDescriptor[]) => {
    const phaseIdsMap = new Map([...(itemsMap.values() || [])].filter(isPhase).map(phase => [phase.typeCode, phase.id]));
    for (const descriptor of constraintDescriptors) {
        const masterPhases = getPhasesByTypecode(itemsMap, phaseIdsMap, descriptor.masterPhases).filter(isMasterPhaseValid);
        const avaliableSlavePhases = getPhasesByTypecode(itemsMap, phaseIdsMap, descriptor.slavePhases);
        const slavePhases = avaliableSlavePhases?.filter(isSlavePhaseValid);
        const isNominativa = isCommessaNominativa(itemsMap);
        if (!!avaliableSlavePhases?.length) {
            const slavePhase = { ...avaliableSlavePhases[0] };
            slavePhase.isSlaveOnConstraint = isNominativa ? true : undefined;
            itemsMap.set(slavePhase.id, slavePhase);
        }
        if (!!masterPhases?.length && !!slavePhases?.length) {
            const slavePhase = { ...slavePhases[0] };
            const slavePhaseBaselineDuration = moment(slavePhase.baselineEndDate).diff(slavePhase.baselineStartDate, 'days');
            const masterPhasesForecastEndDates = masterPhases
                .map(item => item.forecastEndDate as moment.Moment)
                .filter(date => !!date?.isValid());

            if(!isNominativa){
                updateForecast(slavePhase, []);
            }else
            {
                slavePhase.forecastStartDate = moment(getMaxDate(...masterPhasesForecastEndDates));
                slavePhase.forecastEndDate = moment(slavePhase.forecastStartDate).add(slavePhaseBaselineDuration, 'days');
