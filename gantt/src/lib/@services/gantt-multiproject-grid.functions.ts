...
import { GanttItemMultiProject, GanttTaskBars, ItemsMap } from '../@model/gantt-item-multiproject.model';
...
export function resolveCommissionDates(commissionId: number, map: ItemsMap): void {
  ...
  if (!commission?.baselineStartDate?.isValid() && !commission?.baselineEndDate?.isValid()) {
    const startBaseline = commission.childrenIdList?.map(id => map.get(id)?.baselineStartDate).filter(d => d?.isValid());
    const endBaseline = commission.childrenIdList?.map(id => map.get(id)?.baselineEndDate).filter(d => d?.isValid());
...
export function getMinItemDate(item: GanttItemMultiProject): MtDate {
  return moment(
    getMinDate(...[item.baselineStartDate, item.actualStartDate, item.forecastStartDate, item.mapStartDate].filter(d => d?.isValid()))
  );
}

export function getMaxItemDate(item: GanttItemMultiProject): MtDate {
  return moment(getMaxDate(...[item.baselineEndDate, item.actualEndDate, item.forecastEndDate, item.mapEndDate].filter(d => d?.isValid())));
}
...
export const calculateBars = (
  ...
  const hasLastReauth = item.lastReauthStartDate?.isValid() && item.lastReauthEndDate?.isValid();
  const hasForecast = item.forecastStartDate?.isValid() && item.forecastEndDate?.isValid();

  const hasActualStart = item.actualStartDate?.isValid();
  const actualEnd = item.actualEndDate?.isValid() && item.actualEndDate || moment();
  const lastReauthBar = hasLastReauth &&
    calculateBar(
