...
export const hide = <T extends GanttItemMultiProject>(itemId: number, map: Map<number, T>): void => {
  const item = map.get(itemId);
  if (item) {
      const _item = { ...item };
...
export const collapseAll = <T extends GanttItemMultiProject>(map: Map<number, T>): void => {
...
export const sort =
    <T extends GanttItemMultiProject>(items: T[], projectId: number, map: Map<number, T>): T[] => {
...
export const spreadItem = <T extends GanttItemMultiProject>(item: T, map: Map<number, T>, accumulator: T[] = []): T[] =>
...
export const toggleCollapse = <T extends GanttItemMultiProject>(itemId: number, map: Map<number, T>): void => {
  const item = map.get(itemId);
  if (item && item.collapsable) {
      const _item = {...item };
      if (item.collapsed) {
...
const setActivation = <T extends GanttItemMultiProject>(itemId: number, map: Map<number, T>, value: boolean): void => {
  const item = map.get(itemId);
  if (item?.level > 1) {
      const _item = { ...item};
...
export const toggleActivation = <T extends GanttItemMultiProject>(itemId: number, map: Map<number, T>): void => {
  const item = map.get(itemId);
  if (item) {
...
const forbiddenTypeCodes = new Set(['PUB_AUTHORIZATIONS', 'PRI_AUTHORIZATIONS', 'CONTRACT']);
...
export function getGridHeader(map: ItemsMap): string[] {
  ...
  if (start?.isValid() && end?.isValid()) {
      ...
      return new Array(monthsDuration).fill(1).map((val, index) => startNormalized.add(!!index && val, 'months').format('MM/YY'));
  } else {
      return new Array(24).fill(1).map((val, index) => moment().startOf('month').add(!!index && val, 'months').format('MM/YY'));
  }
}

export function getStartDate(project: GanttItemMultiProject): MtDate {
  return moment(project.baselineStartDate).isValid() && moment(project.forecastStartDate).isValid() ?
    getMinDate(moment(project.baselineStartDate), moment(project.forecastStartDate)) :
    moment().startOf('month');
}

export function getEndDate(project: GanttItemMultiProject): MtDate {
    return moment(project.baselineEndDate).isValid() && moment(project.forecastEndDate).isValid() ?
      getMaxDate(moment(project.baselineEndDate), moment(project.forecastEndDate)) :
      moment().endOf('month');
  }
...
export function resolveProjectDates(projectId: number, map: ItemsMap): void {
  ...
  const phaseList: GanttItemMultiProject[] =
    [...map.values()].filter(item => item.typeCode !== 'RESTORATIONS').filter(item => item.id !== projectId);
  const startDateMap: DateType = moment(getMinDate(...phaseList.map(item => item.mapStartDate as moment.Moment)));
  const endDateMap: DateType = moment(getMaxDate(...phaseList.map(item => item.mapEndDate as moment.Moment)));
  project = {
      ...project,
...
export const getProject = (items: GanttItemMultiProject[]): GanttItemMultiProject => items.find(item => item.level === 0);

export const toMap =
    <T extends GanttItemMultiProject>(items: T[]): Map<number, T> =>
