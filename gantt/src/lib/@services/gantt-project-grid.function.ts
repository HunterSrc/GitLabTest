...
const calculateEffectiveEndDate = (item: GanttItem, hasPlannedStart: boolean): moment.Moment => {
    if (!!item.hasActualStartDate && hasPlannedStart) {
        return item.progress < 100 ? moment() : moment(item.plannedEndDate?.value);
    }
...
export const calculateBars = (
    ...
    inactiveColor: string): GanttTaskBars => {
        ...
        const hasMap = (item.mapStartDate as MtDate)?.isValid() && (item.mapEndDate as MtDate)?.isValid();
        const hasStartPlanned = !!((item.plannedStartDate?.value as MtDate)?.isValid());
        const hasPlanned = hasStartPlanned && (item.plannedEndDate?.value as MtDate)?.isValid();
        ...
        const plannedBar = hasPlanned &&
                                calculateBar(
                                    ...
                                    gridStartDate, resolveColor(forecastColor),
                                    sameAsToday(item.plannedStartDate?.value as MtDate));
        const effectiveBar = effectiveEnd.isValid() &&
                                calculateBar(
                                    item.plannedStartDate?.value as MtDate,
                                    effectiveEnd,
                                    gridStartDate, resolveColor(actualColor),
                                    sameAsToday(item.plannedStartDate?.value as MtDate));
        return {
            id: item.id,
            gridStartDate,
