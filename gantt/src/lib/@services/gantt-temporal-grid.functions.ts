import { getMinDate, getMaxDate, MtDate } from './gantt-execution-rollup.functions';
...
export function getGridHeader(itemsMap: ItemsMap): string[] {
    ...
    const end = getMaxDate(...[ ...itemsMap.values()]
                    .map(item => item.baselineEndDate as moment.Moment)
                    .filter(item => item?.isValid()));
    const endNormalized = end && moment(end).startOf('month');
    const monthsDuration = endNormalized?.diff(startNormalized, 'months') + 2 || 24;
...
export function getStartDate(itemsMap: ItemsMap): MtDate {
    const start = getMinDate(...[ ...itemsMap.values()]
                    .map(item => item.baselineStartDate as moment.Moment)
                    .filter(item => item?.isValid()));
    return start && moment(start).startOf('month') || moment().startOf('month');
}
