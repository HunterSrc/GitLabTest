...
import { applyUpdaterTemporal, ganttRowToUpdater } from '../@model/gantt-item.mapper';
...
export class GanttTemporalService {
    ...
    private itemsSubject: ReplaySubject<GanttItemTemporal[]> = new ReplaySubject();
    ...
    init(commissionObjects: GanttItemTemporal[]): void {
        ...
        this.commissionId = commissionObjects.find(item => item.typeCode === 'COMMISSION')?.id;
    ...
    private getCommissionObjects(): GanttItemTemporal[] {
        ...
        return Array.from(this.itemsMap.values()).filter(item => item.id !== projectId);
    }
