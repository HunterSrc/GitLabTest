import { AfterContentChecked, ChangeDetectionStrategy, ChangeDetectorRef, Component, ElementRef, Input, OnDestroy, OnInit, ViewChild } from '@angular/core';
...
const stringToDate = (value: string): moment.Moment => moment(value, DATE_PATTERN).isValid() ? moment(value, DATE_PATTERN) : null;

const dateToString = (value: moment.Moment): string => value?.isValid() ? value.format(DATE_PATTERN) : null;
...
export class TasksComponent implements OnInit, AfterContentChecked, OnDestroy {
  ...
  initForms(): void {
    ...
    this.itemsForms.valueChanges.pipe(tap(this.updateMap.bind(this))).subscribe();
    this.tableSetup();
  }
  ...
  updateMap<T extends FormValue>(_value: any): void {
    this.itemsForms.controls.forEach(control => {
      ...
      if (item) {
        const _item = { ...item };
        const adjustStartMonth = control.get('duration')?.dirty && !(+value.startMonth);
        _item.startMonth = adjustStartMonth ? 1 : ganttFn.fixNegativeOrDecimalValue(+value.startMonth);
        _item.duration = ganttFn.fixNegativeOrDecimalValue(+value.duration);
        _item.mapStartDate = stringToDate(value.startDateMap);
        _item.mapEndDate = item.isMilestone ? stringToDate(value.startDateMap) : stringToDate(value.endDateMap);
        /** SET MANUAL FLAGS */
  ...
  isPhase = (index: number): boolean => ganttFn.isPhase(this.getItemAtIndex(index));

  getItemAtIndex = (index: number): GanttItemAtemporal => this.itemsMap.get(this.itemsIds[index]);

  showActivation = (index: number): boolean => !this.isReadonly(index) && this.isPhase(index);
  ...
  isReadonly(index: number): boolean {
    ...
    const editable = !!this.editableTypes?.has(TypeCodeEnum[typeCode]) || typeCode === 'NR_PROJECT';
