...
const toUpdater = (item: GanttItemExecution, itemsMap: ItemsMap): ItemUpdater => {
  const updater: ItemUpdater = {
    ...
    forecastStartDate: (item.forecastStartDate as moment.Moment)?.toDate() || null,
  ...
  if (item.parent != null && itemsMap.has(item.parent)) {
    ...
    updater.parentActualStartDate = (parent.actualStartDate  as moment.Moment)?.toDate() || null;
    updater.parentActualEndDate = (parent.actualEndDate  as moment.Moment)?.toDate() || null;
...
export class GanttExecutionComponent implements OnInit, OnDestroy {
  ...
  @Output() updateCurrentHeaderColumns: EventEmitter<any> = new EventEmitter<any>();
  ...
  constructor(private ganttService: GanttExecutionService, private dialog: MatDialog) {}

  ngOnInit(): void {
    this.ganttSubscription = this.ganttService.getItems().pipe(
        mergeMap(_ => this.getItems()),
        filter(this.propagateChange.bind(this)),
        map(item => toUpdater(item, this.getMap())),
        delay(500),
        distinctUntilChanged((iu1: ItemUpdater, iu2: ItemUpdater) => MD5(iu1) === MD5(iu2)),
        tap(updater => this.updateCurrent.emit(updater))
      )
      .subscribe();
  }

  propagateChange = (item: GanttItemExecution): boolean => this.currentIds.includes(item.id) || item.id === this.commissionId;
  ...
  public getRows(): GanttRow[] {
    const isEditable = (item: GanttItemExecution): boolean => this.editableTypes?.has(TypeCodeEnum[item.typeCode]);
    return this.ganttService.getItemsBuffer().filter(isEditable).map(mapExecutionRow);
  }
  ...
  onColumnsClick = (): void => {
    this.columnsDialogSub = this.dialog.open(ColumnsDialogComponent, {
      width: '35%',
      autoFocus: false,
  ...
  onAssocia = (event: any): void => {
    this.associa.emit(event);
  }
  
  onCreaPermessi = (event: any): void => {
    this.creaPermessi.emit(event)
  }
}
