...
import { GanttItemExecution, GanttTaskBars } from '../../@model/gantt-item-execution.model';
...
import { GanttExecutionService, Header } from '../../@services/gantt-execution.service';
...
export class GridComponent implements OnDestroy, AfterViewInit {
  ...
  ngAfterViewInit(): void {
    this.headerSubscription = this.ganttService.getHeader().subscribe(header => this.gridHeader = header);
  ...
  initGrid(): void {
    const _rowSize = this.isNominativa ? config.ROWSIZE : config.ROWSIZE_SMALL;
    ...
    const applyCss = (style: CSSStyleDeclaration, rows: number[], cols: number[], rowSize: number, colSize: number) => {
      ...
      style.height = rows.reduce((prev: number, curr: number) => prev + curr * rowSize, 0) + 'px';
      style.gridTemplateRows = rows.map(fr => `${fr}fr`).join(' ');
    };

    const itemRows: number[] = this.items
      .filter((_, index: number) => this.showRow(index))
      .map((item: GanttItemExecution) => isLowLevel(item) ? config.HEIGHT_MULTIPLIER : 1);
    ...
    style.height = itemRows
      ...
      .reduce((prev: number, curr: number) => prev + curr * _rowSize / config.ROWSPAN, 0) + 'px';
    style.gridTemplateRows = itemRows
      .reduce((prev: string, curr: number) =>
        prev + (curr === 1
          ? ` ${_rowSize / (config.ROWSPAN - 1)}px`.repeat(3)
          : ` ${_rowSize * config.HEIGHT_MULTIPLIER / config.ROWSPAN}px`.repeat(config.ROWSPAN)), '');
    ...
    this.items
      ...
      .forEach((item: GanttItemExecution, index: number) => {
        ...
        if (this.isNominativa && !!item.isActive) {
          ...
          if (item.level <= 2 && !!item.isActive) {
            this.drawItem(item, rowIndex);
          }
        }
      });
    const currentDate = moment().startOf('day');
    if (!(currentDate.isBefore(this.startDate) || currentDate.isAfter(this.endDate))) {
  ...
  showHeader(): void {
    if (this.header?.nativeElement) {
      (this.header.nativeElement as HTMLElement).style.visibility = 'visible';
    }
  }

  private drawItem(item: GanttItemExecution, row: number): void {
    const bars: GanttTaskBars =
      gridFn.calculateBars(
        item,
        this.startDate,
  ...
  private drawBars(bars: GanttTaskBars, row: number, isSmall: boolean): void {
    ...
    if (this.isNominativa) {
      ...
      forecastActualBarRow = _row + (isSmall ? 1 : this.showProjectBar ? 3 : 2);
    } else {
    ...
    if (!isSmall) {
      if (bars.mapBar && this.isNominativa) {
        this.drawBar(bars.id + '_map', mapBarRow, bars.mapBar.start, bars.mapBar.end, bars.mapBar.color);
      }
    ...
    if (bars.forecastBar) {
        this.drawBar(bars.id + '_forecast',
    ...
    if (bars.actualBar) {
      if(bars.actualBar.dateStart?.format('DD/MM/YYYY') === bars.actualBar.dateEnd?.format('DD/MM/YYYY') 
          && moment()?.format('DD/MM/YYYY') === bars.actualBar.dateStart?.format('DD/MM/YYYY') ){ 
        bars.actualBar.start = bars.actualBar.start-1;
      }
      this.drawBar(bars.id + '_actual',
        forecastActualBarRow,
  ...
  private drawBar(  taskName: string,
      ...
      if (this.isNominativa) {
        ...
        divTask.style.marginTop = isUpper ? '0px' : this.showProjectBar ? '5px' : '-6px';
      }
  ...
  showRow = (index: number): boolean => this.showZeroWeightRows || this.items[index].weight > 0;
}
