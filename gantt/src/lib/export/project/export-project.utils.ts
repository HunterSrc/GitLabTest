...
export const getGanttRowLimit = (input: GanttRow) => Math.max(getEnd(input.map), getEnd(input.planned));

export const shiftIntervals = <T extends Intervals>(input: T, delta: number = 0): T => ({
    ...input,
...
export const getIntervals = (item: GanttItem, gridStart: moment.Moment): Intervals => {
    const gridStartNormalized = moment(gridStart).startOf('month');
    const startDateMap = normalizeDate(moment(item.mapStartDate)).startOf('month');    
    const endDateMap = normalizeDate(moment(item.mapEndDate)).startOf('month');
    const startDatePlanned = normalizeDate(moment(item.plannedStartDate?.value)).startOf('month');
    const endDatePlanned = normalizeDate(moment(item.plannedEndDate?.value)).startOf('month');

    const mapStart = startDateMap.isValid() ?
            moment(startDateMap).diff(gridStartNormalized, 'months') : 0;
    const plannedStart = startDatePlanned.isValid() ?
            moment(startDatePlanned).diff(gridStartNormalized, 'months') : 0;
    // Normalize the planned date as today  
    const todoStart = (!item.hasActualStartDate && item.progress === 0 ?
        adjustNotStartedBar(item?.plannedStartDate?.value as moment.Moment) :
        normalizeToday()).diff(gridStartNormalized, 'months');
    const doneStart = plannedStart;
    const mapDiffDate = moment(endDateMap).diff(startDateMap, 'months');
    const plannedDiffDate = moment(endDatePlanned).diff(startDatePlanned, 'months');
    const mapDuration = endDateMap.isValid() && startDateMap.isValid() ?
            (mapDiffDate >0 ? mapDiffDate  : 1 ): 0;

    const plannedDuration = endDatePlanned.isValid() && startDatePlanned.isValid() ?
            (plannedDiffDate >0 ? plannedDiffDate : 1) : 0;
    ...
    if (item.progress === 100) {
        ...
        if (!!item.progress || !!item.hasActualStartDate) {
...
const normalizeDate = (date: moment.Moment): moment.Moment => ( moment(date).date() > 15 ? moment(date).add(1, 'months') : moment(date)).startOf('month');

const normalizeToday = (): moment.Moment => normalizeDate(moment());

const adjustNotStartedBar = (date: moment.Moment): moment.Moment => 
date?.isValid() && date.format('DD/MM/YYYY') !== moment().format('DD/MM/YYYY') ? normalizeDate(moment(date)) : normalizeToday()
