...
const getRectStartX = (phasePage: number[]): number => phasePage?.length > 0 ?
    GANTT_START_X + (MONTH_WIDTH * (phasePage[0] - 1)) : 0;
...
export class GanttTemporalExportComponent implements AfterViewInit {
    ...
    getProjectDuration = (): number => this.pageMap.get(this.projectCode).duration;
    ...
    getStartTargetDaMap = (color: Color): number => getRectStartXByColor(this.getProjectPage(), color);
    getWidthTargetDaMap = (color: Color): number => getRectWidthByColor(this.getProjectPage(), color);

    getStartIngegneria = (color: Color): number => getRectStartXByColor(this.getIngegneriaPage(), color);
    getWidthIngegneria = (color: Color): number => getRectWidthByColor(this.getIngegneriaPage(), color);

    getStartPermessi = (color: Color): number => getRectStartXByColor(this.getPermessiPage(), color);
    getWidthPermessi = (color: Color): number => getRectWidthByColor(this.getPermessiPage(), color);

    getStartProcurementMateriali = (color: Color): number => getRectStartXByColor(this.getProcurementMaterialiPage(), color);
    getWidthProcurementMateriali = (color: Color): number => getRectWidthByColor(this.getProcurementMaterialiPage(), color);

    getStartProcurementLavori = (color: Color): number => getRectStartXByColor(this.getProcurementLavoriPage(), color);
    getWidthProcurementLavori = (color: Color): number => getRectWidthByColor(this.getProcurementLavoriPage(), color);

    getStartCostruzione = (color: Color): number => getRectStartXByColor(this.getCostruzionePage(), color);
    getWidthCostruzione = (color: Color): number => getRectWidthByColor(this.getCostruzionePage(), color);

    getStartRipristini = (color: Color): number => getRectStartXByColor(this.getRispristiniPage(), color);
    getWidthRipristini = (color: Color): number => getRectWidthByColor(this.getRispristiniPage(), color);

    private getDuration = (key: string): string => getPaddedString(!!this.isActive(key) ? this.pageMap?.get(key)?.duration : 0);
    private getDelay = (key: string): string => getPaddedString(!!this.isActive(key) ? this.pageMap?.get(key)?.delay : 0);
    ...
    hasMessaInEsercizioMap = (): boolean => this.getMessaInEsercizioPage().map[0] >= 0;
    hasMessaInEsercizioPlanned = (): boolean => this.getMessaInEsercizioPage().done[0] >= 0;
    ...
    getStartMessaInEsercizioPlanned = (): string => `m ${getMilestoneX(this.pageMap?.get('EE').done)},147 2.5,2.5 -2.5,2.5 -2.5,-2.5 z`;
    ...
    getYear = (index: number): number => this.startYear + (3 * (this.currentPage - 1)) + index - 1;

    private isActive = (key: string) =>  this.pageMap.has(key) ? !!this.pageMap.get(key).isActive : false;

    getIngegneriaColor = (color: string): string => `fill:${ this.isActive('ING') ? color : '#cccccc90'}`;
    getPermessiColor = (color: string): string => `fill:${ this.isActive('PS') ? color : '#cccccc90'}`;
    getProcurementMaterialiColor = (color: string): string => `fill:${ this.isActive('MAT') ? color : '#cccccc90'}`;
    getProcurementLavoriColor = (color: string): string => `fill:${ this.isActive('PL') ? color : '#cccccc90'}`;
    getCostruzioneColor = (color: string): string => `fill:${ this.isActive('COS') ? color : '#cccccc90'}`;
    getRipristiniColor = (color: string) => `fill:${ this.isActive('RP') ? color : '#cccccc90'}`;
    getMessaInEsercizioColor = (color: string) => `fill:${ this.isActive('EE') ? color : '#cccccc90'}`;
}
