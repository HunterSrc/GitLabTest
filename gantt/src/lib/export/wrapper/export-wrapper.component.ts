...
export class GanttExportWrapperComponent implements AfterViewInit {
    ...
    constructor(public dialogRef: MatDialogRef<GanttExportWrapperComponent>, @Inject(MAT_DIALOG_DATA) private data: DialogData) {
        const projectStartDate = data.mapStartDate?.isValid() ? moment(data.mapStartDate) : moment();
        this.currentDate = Moment().format('DD/MM/YYYY');
        this.startPadding = !!data?.isAtemporal ? 0 : (projectStartDate.month());
        if (!!data?.items) {
            this.project = [...data.items].find(item => item.level === 0);
            this.phaseDurationMap = new Map([...data.items].map(
                item => [
                    item.externalCode,
                    {
            ...
            [...this.phaseDurationMap.entries()].forEach(entry => {
                this.splittedPhaseMap.set(entry[0], splitActivity(entry[1], this.ranges, true));
            });
        }
        console.log(this.splittedPhaseMap);
    }
    ...
    get startYear(): number {
        return this.data?.mapStartDate?.isValid() ? this.data?.mapStartDate?.year() : moment().year();
    }
    ...
    getPageMap = (page: number) =>
        !!this.splittedPhaseMap ?
            new Map([...this.splittedPhaseMap.entries()].map(entry => [entry[0], entry[1][page - 1]])) :
            new Map()
