...
import { ProjectDates } from 'projects/mon-impianti/src/app/@services/dtos/progetto/progetto.detail.dto';
...
export class GanttProjectComponent implements OnInit, OnDestroy {
  ...
  @Input() set projectGantt(data: { projectGantt: ProjectGantt, editable: boolean}) {
    if (!!(data?.projectGantt?.ganttRows?.length)) {
      this.ganttService.init(data.projectGantt.ganttRows, data.projectGantt.constraints, data.editable);
    }
  }
