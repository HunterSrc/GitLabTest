...
import { GanttProjectService, Header } from '../../@services/gantt-project.service';
...
export class GridComponent implements AfterViewInit, OnDestroy {
  ...
  ngAfterViewInit(): void {
    this.itemsSubscription = this.ganttService.getHeader().subscribe(header => this.gridHeader = header);
    this.headerSubscription = this.ganttService.getItems().subscribe(items => this.updateGrid(items));
  }
  ...
  private drawItem(item: GanttItem, row: number): void {
    const bars: gridFn.GanttTaskBars =
      gridFn.calculateBars(
        item,
        this.startDate,
        config.BASELINE_COLOR,
        item.progress === 100 ? config.COMPLETED_COLOR : config.ACTUAL_COLOR,
        item.progress === 100 ? config.COMPLETED_COLOR : config.PROGRESS_COLOR,
        config.INACTIVE);
  ...
  private drawBars(bars: gridFn.GanttTaskBars, row: number): void {
    ...
    if (bars.mapBar) {
      this.drawBar(bars.id + '_map', _row, bars.mapBar.start, bars.mapBar.end, true, bars.mapBar.color);
    }
    if (bars.plannedBar) {
    ...
    if (bars.effectiveBar) {
      this.drawBar(bars.id + '_progress',
                  _row + 1,
  ...
  private drawBar(  taskName: string,
                    ...
                    isUpper: boolean,
                    color: string = config.BASELINE_COLOR): void {
