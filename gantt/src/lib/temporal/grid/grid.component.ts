...
import { isCommission, isProject } from '../../@services/gantt-execution-task.functions';
...
export class GridComponent implements AfterViewInit, OnDestroy {
  ...
  initGrid(): void {
    ...
    const applyCss = (style: CSSStyleDeclaration, rows: number[], cols: number[], rowSize: number, colSize: number) => {
      ...
      style.width =  this.cols.length <= 24 ? config.FULL_WIDTH : `${config.COLSIZE * this.cols.length}px`;
  ...
  updateGrid(items: GanttItemTemporal[]): void {
    ...
    this.initGrid();
    this.items.forEach(
      (item, row) => this.drawItem(item, row + 1)
    );
  ...
  private drawItem(item: GanttItemTemporal, row: number): void {
    if ((item.baselineStartDate as moment.Moment)?.isValid() && (item.baselineEndDate as moment.Moment)?.isValid()) {
      let color = BASELINE_COLOR;
      if (isCommission(item)) { color = COMMISSION_COLOR; }
      if (isProject(item)) { color = PROGRESS_COLOR; }
      if (!item.isActive) { color = INACTIVE; }
      const bar: GanttBar = calculateBar(
        item.baselineStartDate as moment.Moment,
        item.baselineEndDate as moment.Moment,
        this.startDate,
        color);
      this.drawBar(item.id + '_baseline', row, bar.start, bar.end, bar.color);
    }
  }
