...
export const applyUpdaterExecution = (
    ...
    item.actualStartDate =
        updater.actualStartDate != null
            ? moment(updater.actualStartDate)
            : null;
    item.actualEndDate =
    ...
    if (item.level <= 2) {
        ...
        item.baselineEndDate =
            updater.baselineEndDate != null
                ? moment(updater.baselineEndDate)
                : null;
        ...
        item.forecastEndDate =
            updater.forecastEndDate != null
                ? moment(updater.forecastEndDate)
                : null;
        ...
        item.mapStartDate =
            ...
            item.mapStartDate;
        item.mapEndDate =
            (updater.mapEndDate && moment(updater.mapEndDate)) ||
            item.mapEndDate;
        item.lastReauthStartDate =
            (updater.lastReauthStartDate &&
                moment(updater.lastReauthStartDate)) ||
            item.lastReauthStartDate;
...
export const modifiedRowsToAtemporalGantt = (
    ...
    const itemsMap: AtemporalMap = toMapAtemporal([
        ...(gantt?.ganttRows || [])
    ]);
    modifiedRows.forEach((row) => {
        ...
        if (mapItem) {
            ...
            item.mapStartDate =
                (row.mapStartDate && moment(row.mapStartDate)) ||
                item.mapStartDate;
            item.mapEndDate =
...
export const modifiedRowsToExecutionGantt = (
    ...
    const itemsMap: ExecutionMap = toMapExecution([
        ...(gantt?.ganttRows || [])
    ]);
    modifiedRows.forEach((row) => {
        const mapItem = itemsMap.get(row.id);
...
export const applyUpdaterMultiProject = (
    ...
    if (item.level <= 2) {
        ...
        item.baselineEndDate =
            ...
            null;
        item.forecastStartDate =
            (!!updater.forecastStartDate &&
                moment(updater.forecastStartDate)) ||
            null;
        item.forecastEndDate =
            (!!updater.forecastEndDate && moment(updater.forecastEndDate)) ||
            null;
        item.mapStartDate =
            (!!updater.mapStartDate && moment(updater.mapStartDate)) ||
            item.mapStartDate;
...
export const mapAtemporalRows = (
    ...
    const fnMapper = (_input: GanttRow): ProjectDates<moment.Moment> => ({
        ...
        mapStartDate: _input.mapStartDate?.isValid()
            ...
            : null,
        mapEndDate: _input.mapEndDate?.isValid()
            ? moment(_input.mapEndDate)
            : null,
