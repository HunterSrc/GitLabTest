...
export class GanttAtemporalService {
    ...
    recalculateMap(sourceMap: ItemsMap, isAtemporal: boolean): ItemsMap {
        ...
        const result = new Map(
            ganttFn
                .computeGantt(items, isAtemporal)
                .map((item) => [item.id, item])
        );
        const ganttStartDate = moment(ganttFn.getStartDate(items)).startOf(
            'month'
        );
        ...
        for (const constraintDescriptor of this.projectConstraintsDescriptors) {
            ...
            if (constraint) {
                ...
                const constraintResult =
                    resolveEndToStartConstraint(constraint);
                if (slavePhase) {
                    applyEndToStartConstraint(
                        slavePhase,
                        constraintResult,
    ...
    updateLockedTypeCodeList(): void {
        ...
        for (const constraintDescriptor of this.projectConstraintsDescriptors) {
            if (
                ...
                constraintDescriptor.slavePhases?.forEach((item) =>
                    locked.add(item)
                );
            }
        }
