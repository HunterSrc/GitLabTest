...
export class GanttAtemporalComponent implements OnInit, OnDestroy {
    ...
    @Input() set items(ganttObject: GanttObject<GanttItemAtemporal>) {
        if (ganttObject?.ganttRows) {
            this.ganttService.init(
                ganttObject.ganttRows,
                ganttObject.constraints,
                ganttObject.constraintVersion
            );
        }
    }
    ...
    @Output() isAtemporalChange: EventEmitter<boolean> =
    ...
    public getRows(): GanttRow[] {
        ...
        const ganttRows: GanttRow[] = bufferItems.map((item) =>
            mapAtemporalRow(item)
        );
        return ganttRows;
    }
