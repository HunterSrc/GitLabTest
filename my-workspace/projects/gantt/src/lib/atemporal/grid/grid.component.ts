...
export class GridComponent implements AfterViewInit, OnDestroy {
  ...
  cols: string[] = Array(config.MIN_COLS)
    .fill(1)
    .map((v, i) => v + i);
  ...
  initGrid(): void {
    ...
    const applyCss = (
      ...
      style.width =
        cols.length <= 24 ? config.FULL_WIDTH : `${cols.length * colSize}px`;
  ...
  private drawTask(item: GanttItemAtemporal, row: number): void {
    ...
    if (item.startMonth && item.duration) {
      ...
      this.drawBar(
        ...
        row + 1,
        item.startMonth,
        item.startMonth + item.duration,
        item.isActive ? color : config.INACTIVE
      );
    }
  }

  private drawItem(item: GanttItemAtemporal, row: number): void {
    return (item.isMilestone ? this.drawMilestone : this.drawTask).call(
      this,
      item,
      row
    );
  ...
  private drawMilestone(item: GanttItemAtemporal, row: number): void {
    ...
    divMilestone.style.backgroundColor =
      item.isActive && !!item.startMonth ? config.PHASE_COLOR : config.INACTIVE;
