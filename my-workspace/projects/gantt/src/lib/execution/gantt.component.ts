...
export class GanttExecutionComponent implements OnInit, OnDestroy {
    ...
    @Output() projectStructureClick: EventEmitter<any> =
        new EventEmitter<any>();
    @Output() updateCurrentHeaderColumns: EventEmitter<any> =
        new EventEmitter<any>();
    @Output() hideShowProjectClick: EventEmitter<null> =
        new EventEmitter<null>();

    @Input() currentHeaderColumns: Set<string>;
    get currentHeaderColumnsList(): string[] {
        return (
            !!this.currentHeaderColumns && Array.from(this.currentHeaderColumns)
        );
    }
    ...
    ngOnInit(): void {
        ...
        this.ganttSubscription = this.ganttService
            ...
            .pipe(
                ...
                delay(500),
                distinctUntilChanged(
                    (iu1: ItemUpdater, iu2: ItemUpdater) =>
                        MD5(iu1) === MD5(iu2)
                ),
