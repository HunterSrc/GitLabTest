  ...
  implements OnInit, AfterContentChecked, OnDestroy, OnChanges
{
  ...
  ngOnInit(): void {
    ...
    this.principalStore
      .pipe(take(1))
      .subscribe((res) => (this.role = res.principal.principal.activeRoleCode));
  }
  ...
  ngOnChanges(changes: SimpleChanges) {
    ...
    this.hasWeightError =
      this.ganttRows.filter((el) => this.elementHasWeightError(el)).length > 0;
  }
  ...
  private getTaskForm(item: GanttItemExecution): FormGroup {
    const dateToString = (value: DateType): string | null =>
      isMoment(value) && !!value?.isValid() ? value.format('DD/MM/YYYY') : null;
  ...
  initForms(): void {
    ...
    this.itemsForms.valueChanges
      .pipe(distinctUntilChanged(equalsValues), tap(this.updateMap.bind(this)))
      .subscribe();
  }
  ...
  public hasHighlightError(index: number): boolean {
    ...
    const totalWeight = [...siblingsSet].reduce(
      (prev, curr) =>
        prev + (this.itemsMap.get(curr) ? this.itemsMap.get(curr).weight : 0),
      0
    );
  ...
  showRow(index: number) {
    return !this.showZeroWeightRows || this.getItemAtIndex(index).weight > 0;
  }
  ...
  isStarted = (index: number): boolean => {
    ...
    return (
      !!item &&
      ((item.actualStartDate as moment.Moment)?.isValid() || !!item.progress)
    );
  };
  ...
  isToggleDisabled(index: number) {
    ...
    if (odlListActive.length > 1) {
      const sum = odlListActive.reduce((a, b) => a + b.weight, initialValue);
      return sum !== 100;
    }
    return false;
  }
      ...
      this.detailStore
        ...
        .pipe(
          ...
          switchMap((gantt) => {
            ...
            const ganttToSave = {
              modifiedRows: this.ganttRows.filter(this.isEditable),
    ...
    if (this.showStandardActivity(index)) {
      ...
      this.matDialog
        ...
        .pipe(
          ...
          map(([data, gantt]) => {
            if (!data) {
              this.ganttExecutionService.restoreActivationBackup(odlItem.id);
              return null;
            }
        ...
        .subscribe((response) => {
          if (response) {
            ...
            if (currentWireframe && odlItem?.subjectDetail) {
              this.router.navigate([odlItem.subjectDetail]).finally(() => {
                this.overlaySpinnerSvc.notLoading();
              });
            } else {
              this.detailStore.dispatch(
                SuccessFetchGanttAction(dto2stateMapper(response))
              );
              this.ganttExecutionService.init2(dto2stateMapper(response));
              this.overlaySpinnerSvc.notLoading();
            }
            this.changeOdlConstructionInProgress = false;
          }
  ...
  sumWeightsActive(odlItem: GanttItemExecution) {
    const acc = 0;
    const odlList = this.ganttRows.filter((el) => el.parent === odlItem.parent);
