...
export class GanttProjectExportWrapperComponent implements AfterViewInit {
  ...
  constructor(
    public dialogRef: MatDialogRef<GanttProjectExportWrapperComponent>,
    ...
    this.projectItemsMap = new Map(
      intervals.map((item) => [item.externalCode, item])
    );
  ...
  private getPageMap = (
    ...
    map
      ? new Map(
          [...map.entries()].map((entry) => [entry[0], entry[1][page - 1]])
        )
      : new Map();

  private getMapsPageMap = (page: number): Map<string, number[]> =>
    this.getPageMap(this.projectMapsMap, page);

  private getTodosPageMap = (page: number): Map<string, number[]> =>
    this.getPageMap(this.projectTodosMap, page);

  private getDonePageMap = (page: number): Map<string, number[]> =>
    this.getPageMap(this.projectDoneMap, page);
