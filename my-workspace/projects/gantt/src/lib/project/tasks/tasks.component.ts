...
export class TasksComponent implements OnInit, AfterViewInit, OnDestroy {
    ...
    if (this.isUnderReautorization === true) {
      ...
      editable = [
        +(
          ...
          this.isDifferentFromProjectStart(item.mapStartDate, item.typeCode) &&
          item.level > 0
        ),
  ...
  private validateMap(): any[] {
    const checkDate = (date: Editable<DateType>, id: number, start = true) =>
      this.isUnderReauth
        ? this.editablePlannedDates.get(id)[start ? 0 : 1] === 1 &&
          dateValidator(date?.value as moment.Moment)
        : !!date?.editable && dateValidator(date?.value as moment.Moment);
  ...
  isDifferentFromProjectStart(date: DateType, typeCode: string): boolean {
    const constraintVersion = this.ganttProjectService.getConstraintVersion();
    ...
    for (const slave of slaves) {
      ...
      if (slaveElement) {
        ...
        if (
          ...
          slaveItem.plannedStartDate.value = this.calculateStartDate(slaveItem);
          slaveItem.plannedEndDate.value = this.calculateEndDate(slaveItem);
          this.itemsMap.set(slaveItem.id, slaveItem);
          const newSlaves = this.ganttProjectService.getSlaves(slaveItem);
