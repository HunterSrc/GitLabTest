...
import {
    isCommission,
    isProject
} from '../../@services/gantt-temporal-task.functions';
...
export class TasksComponent implements OnInit, AfterViewInit, OnDestroy {
        ...
        const group = this.formBuilder.group(
            {
                ...
                startDateBaseline: dateToString(
                    item.baselineStartDate as moment.Moment
                ),
                endDateBaseline: dateToString(
    ...
    updateMap<T extends FormValue>(_value: any): void {
        ...
        this.itemsForms.controls.forEach((control) => {
            ...
            if (item) {
                ...
                _item.baselineEndDate = item.isMilestone
                    ? stringToDate(value.startDateBaseline)
                    : stringToDate(value.endDateBaseline);
    ...
    isReadonly = (index: number): boolean => {
        ...
        const editable = !!this.editableTypes?.has(
            TypeCodeEnum[this.getItemAtIndex(index)?.typeCode]
        );
        const isProjectOrCommission =
            item && (isProject(item) || isCommission(item));
        return this.locked || !editable || isProjectOrCommission;
    };

    getItemAtIndex = (index: number): GanttItemTemporal =>
        this.itemsMap.get(this.itemsIds[index]);

    showActivation = (index: number): boolean =>
        !this.isReadonly(index) && ganttFn.isPhase(this.getItemAtIndex(index));
